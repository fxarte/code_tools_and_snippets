...
alias ..='cd ..'
alias fnd='myfind'
alias html='cd ../html  2> /dev/null || cd html 2> /dev/null'
alias home='cd /home/felix/projects/portal/html'
alias svns='svn status'
alias svnup='svn update'
alias html='cd html 2>/dev/null || cd ../html 2>/dev/null'

function myfind(){
  find . -type f -not -path "*svn*" -exec grep -l "$1" {} \;
}
function sqldrush(){
 if [ -e "$1" ]; then
  $(drush sql-connect) < $1
 else
  echo "$1" | drush sql-cli
...
function svnplog(){
 ...
 if [ -n "$1" ];then
   ARG="$1"
   svn log --stop-on-copy $ARG | grep -EA2 r[0-9]+ | sed 's/^--//' | sed '/^$/d' | sed '$!N;s/\n/ | /'
 fi
}
function svnbranch(){
 ...
 SVN_FOLDER=${SVN_FOLDER/branches/}
 SVN_FOLDER=${SVN_FOLDER/tags/}
 SVN_FOLDER=${SVN_FOLDER//\// }
 SVN_FOLDER=($SVN_FOLDER)
 if [ -n ${SVN_FOLDER[0]} ]; then
  echo "(${SVN_FOLDER[0]})";
 fi
}
